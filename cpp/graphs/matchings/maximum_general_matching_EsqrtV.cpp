...
int main() {
    ...
    auto run = [](int N, const vector<Edge> &edges) {
        ...
        clock_t end = clock();
        fprintf(stderr, "|V| = %6d, |E| = %7d: %d %.3f sec\n",
    ...
    // random
    {
        const int N = K, M = 2.0 * N;
        uniform_int_distribution<int> urand(1, N);
        vector<Edge> edges(M);
        for (int i = 0; i < M; ++i) {
            int u = urand(mt), v = urand(mt);
            edges[i] = {u, v};
        }
        run(N, edges);
    }
    ...
    {
        const int N = K, M = 2.0 * N;
        uniform_int_distribution<int> urand(1, N / 2), urand2(1, N - N / 2);
        vector<Edge> edges(M);
        for (int i = 0; i < M; ++i) {
            int u = urand(mt), v = urand2(mt) + N / 2;
            edges[i] = {u, v};
        }
        run(N, edges);
    }
