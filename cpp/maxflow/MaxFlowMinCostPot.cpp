...
int prio[maxnodes], pot[maxnodes], curflow[maxnodes], edge[maxnodes];
...
void bellmanFord(int s) {
    ...
    for (int k = 0; k < nodes && wasChanged; k++) {
        wasChanged = false;
...
pii maxFlowOfMinCost(int s, int t) {
    ...
    while (true) {
        ...
        while (!q.empty()) {
            ...
            for (int e = last[u]; e >= 0; e = previous[e]) {
                int v = head[e];
                int nprio = d + cost[e] + pot[u] - pot[v];
