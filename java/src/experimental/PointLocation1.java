...
public class PointLocation1 {
	...
	public static int[] locatePoints(int[][] polygonX, int[][] polygonY, int[] qx, int[] qy) {
		...
		for (Event event : events) {
			if (event.type == 1) {
				...
				SortedSet<Segment> head = set.headSet(new Segment(event.x, event.y, event.x, event.y, 0), true);
				if (!head.isEmpty() && (head.last().contains(event.x, event.y) || head.size() % 2 == 1)) {
					res[event.queryId] = head.last().polygonId;
				}
				SortedSet<Segment> tail = set.tailSet(new Segment(event.x, event.y, event.x, event.y, 0), false);
				if (!tail.isEmpty() && tail.first().contains(event.x, event.y)) {
					res[event.queryId] = tail.first().polygonId;
				}
			}
		}
		return res;
	...
	public static void main(String[] args) {
		for (int step = 0; step < 10_000; step++) {
			...
			int queries = rnd.nextInt(10) + 1;
			int[] qx = new int[queries];
			int[] qy = new int[queries];
			int[] res1 = new int[queries];
			Arrays.fill(res1, -1);
			for (int i = 0; i < queries; i++) {
				qx[i] = rnd.nextInt((C + 2) * (range + 1)) - range;
				qy[i] = rnd.nextInt((R + 2) * (range + 1)) - range;
