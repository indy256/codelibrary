...
public class DYNALCA_Checker {
	...
	static void genCase(PrintWriter pw) {
		...
		LinkCutTree.Node[] nodes = new LinkCutTree.Node[n];
		for (int i = 0; i < nodes.length; i++) {
			nodes[i] = new LinkCutTree.Node();
		}
		int q = 100000;
		pw.println(n + " " + q);
			...
			while (links == 0 || links < n - 1) {
				...
				while (true) {
					...
					if (x == y)
						continue;
					try {
						LinkCutTree.link(nodes[x], nodes[y]);
						break;
			...
			if (links > 1) {
				...
				while (true) {
					...
					if (x == y)
						continue;
					try {
						LinkCutTree.lca(nodes[x], nodes[y]);
						break;
					} catch (RuntimeException e) {
