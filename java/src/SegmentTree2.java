...
public class SegmentTree2 {
	...
	void pushDelta(int root, int left, int right) {
		...
		value[(2 * root + 1)] = joinValueWithDelta(value[(2 * root + 1)], deltaEffectOnSegment(delta[root], mid - left + 1));
		value[(2 * root + 2)] = joinValueWithDelta(value[(2 * root + 2)], deltaEffectOnSegment(delta[root], right - mid));
	...
	void modify(int from, int to, int delta, int root, int left, int right) {
		if (from == left && to == right) {
			value[root] = joinValueWithDelta(value[root], deltaEffectOnSegment(delta, right - left + 1));
