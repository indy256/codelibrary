...
public class FibonacciHeap {
	...
	public Node getNode(Object element, boolean create) {
		...
		if (node == null && create) {
			node = new Node(element, Integer.MAX_VALUE);
	...
	/**
	 * Decreases the key value for a heap node, given the new value to take on. The structure of the heap may be changed
	...
	/**
	 * Deletes a node from the heap given the reference to the node. The trees in the heap will be consolidated, if
	 * necessary. This operation may fail to remove the correct element if there are nodes with key value -Infinity.
	 * 
	 * <p>
	...
	/**
	 * Inserts a new data element into the heap. No heap consolidation is performed at this time, the new node is simply
	 * inserted into the root list of this heap.
	 * 
	 * <p>
	...
	/**
	 * Removes the smallest element from the heap. This will cause the trees in the heap to be consolidated, if
	 * necessary. Does not remove the data node so that the current key remains stored.
	 * 
	 * <p>
	 * Running time: O(log n) amortized
	 * </p>
	...
	/**
	 * Joins two Fibonacci heaps into a new one. No heap consolidation is performed at this time. The two root lists are
	...
	/**
	 * Performs a cascading cut operation. This cuts y from its parent and then does the same for its parent, and so on
	 * up the tree.
	 * 
	 * <p>
	...
	/**
	 * Consolidates the trees in the heap by joining trees of equal degree until there are no more trees of equal degree
	 * in the root list.
	 * 
	 * <p>
	 * Running time: O(log n) amortized
	...
	/**
	 * Implements a node of the Fibonacci heap. It holds the information necessary for maintaining the structure of the
	 * heap. It also holds the reference to the key value (which is used to determine the heap structure). Additional
	 * Node data should be stored in a subclass.
	 */
	public static class Node {
		 ...
		 */
		public Node(Object userObject, int key) {
	...
	public static void main(String[] args) {
		FibonacciHeap h = new FibonacciHeap();
		Node node1 = h.getNode(1, true);
		h.decreaseKey(node1, 1);
		Node node2 = h.getNode(2, true);
		h.decreaseKey(node2, 2);
		System.out.println(1 == h.min().getKey());
		h.delete(node1);
		System.out.println(2 == h.min().getKey());
	}
}
