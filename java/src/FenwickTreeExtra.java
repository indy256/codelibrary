public class FenwickTreeExtra {
	...
	public FenwickTreeExtra(int n) {
		this.n = n;
		t = new int[n];
	}

	public FenwickTreeExtra(int[] a) {
		n = a.length;
		t = new int[n];
		for (int i = 0; i < n; i++) {
			t[i] += a[i];
	...
	public static void main(String[] args) {
		FenwickTreeExtra t = new FenwickTreeExtra(10);
		t.set(0, 1);
		...
		t = new FenwickTreeExtra(new int[] { 1, 2, 3, 4, 5, 6 });
		for (int i = 0; i < t.n; i++)
			System.out.print(t.get(i) + " ");
		System.out.println();
		t = new FenwickTreeExtra(new int[] { 0, 0, 1, 0, 0, 1, 0, 0 });
